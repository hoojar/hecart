 ...
 * 版本: $Id: mem.php 1278 2017-09-30 06:05:53Z zhangsl $
 *
...
class wcore_mem
{
	...
	public function push($type, $key, $value)
	{
		...
		$incr   = $this->increment("PUSH-{$type}", $key);
		$prefix = $this->_makey("{$type}{$incr}", $key);
		$result = $this->object->set($prefix, $value, $this->_flag);
		if (false === $result)
		{
			$this->decrement("PUSH-{$type}", $key);
		}
	...
	public function &pop($type, $key, $default = false)
	{
		...
		$incr   = $this->increment("POP-{$type}", $key);
		$prefix = $this->_makey("{$type}{$incr}", $key);
		$result = $this->object->get($prefix);
		if (false === $result)
		{
			$this->decrement("POP-{$type}", $key);
		}
		else
		{
	...
	public function halt($msg, $exit = true)
	{
		...
		{
			file_put_contents(DIR_ROOT . '/logs/mem.log', date('Y-m-d H:i:s') . " - Error: {$msg}\n", FILE_APPEND);
		}
